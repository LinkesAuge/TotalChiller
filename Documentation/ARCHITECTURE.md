...
> **Maintenance**: Update this file when you add/remove/move files, API routes, DB tables, or change codebase patterns. Keep entries factual and structural — describe *what* and *where*, not implementation details. If you add a new feature module, add a section under §4 with its files, DB tables, and key patterns.
...
| Date formatting | `date-format.ts` | `formatLocalDateTime()`, `toDateString()` |
