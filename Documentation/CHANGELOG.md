...
- **New shared module `lib/messages/broadcast-targeting.ts`**: Contains `resolveBroadcastRecipients()`, `userMatchesBroadcastTargeting()`, and `canUserReplyToBroadcast()` utilities used across multiple API routes.
- **New component `app/messages/rank-filter.tsx`**: Rank filter dropdown with preset chips for the compose form.
