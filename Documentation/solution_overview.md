...
- Admin routes include data import and data table.
...
- **Backend**: Supabase Postgres with RLS for clan-scoped data and permissions.
...
- **users**: profile, status, language, linked game aliases.
- **clans**: metadata and memberships.
...
- **profiles**: username (case‑insensitive), username_display, display_name.
...
- Middleware guard: `middleware.ts` redirects unauthenticated users to `/home` and enforces admin‑only routes.
...
- `Documentation/supabase_chest_entries.sql` for `chest_entries` table + RLS policies.
  - Includes audit logs, username system, global default clan, and rank column.
...
- Membership table supports inline edits of username, role, email, display name, clan, rank, status.
...
- Tighten RLS for roles/ranks/permissions tables (admin‑only).
- Add search/filter in members table and batch action confirmations.
