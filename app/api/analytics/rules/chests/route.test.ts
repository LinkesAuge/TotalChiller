...
describe("GET /api/analytics/rules/chests", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const req = createTestRequest("/api/analytics/rules/chests", { searchParams: { clan_id: CLAN_ID } });
    const res = await GET(req);
    expect(res.status).toBe(401);
  });

  it("returns 400 for missing clan_id", async () => {
    const req = createTestRequest("/api/analytics/rules/chests");
    const res = await GET(req);
...
describe("POST /api/analytics/rules/chests", () => {
  let mockAuth: ReturnType<typeof createMockAuth>;
  ...
  it("returns 400 for invalid body", async () => {
    const req = createTestRequest("/api/analytics/rules/chests", { method: "POST", body: { clan_id: CLAN_ID } });
    const res = await POST(req);
    expect(res.status).toBe(400);
  });
...
describe("PUT /api/analytics/rules/chests", () => {
  let mockAuth: ReturnType<typeof createMockAuth>;
  ...
  it("returns 400 for missing required fields", async () => {
    const req = createTestRequest("/api/analytics/rules/chests", { method: "PUT", body: {} });
    const res = await PUT(req);
    expect(res.status).toBe(400);
  });
...
describe("DELETE /api/analytics/rules/chests", () => {
  let mockAuth: ReturnType<typeof createMockAuth>;
