...
/**
 * GET /api/analytics/rules/events
 *
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (error) {
      captureApiError("GET /api/analytics/rules/events", error);
    ...
    captureApiError("GET /api/analytics/rules/events", err);
    return apiError("Internal server error.", 500);
  }
}

/**
 * POST /api/analytics/rules/events
 *
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (insertError || !ruleSet) {
      captureApiError("POST /api/analytics/rules/events (rule set)", insertError);
    ...
    if (tiersError) {
      captureApiError("POST /api/analytics/rules/events (tiers)", tiersError);
    ...
    if (event_type_ids.length > 0) {
      ...
      if (junctionError) {
        captureApiError("POST /api/analytics/rules/events (junctions)", junctionError);
        await supabase.from("clan_event_rule_sets").delete().eq("id", ruleSetId);
    ...
    captureApiError("POST /api/analytics/rules/events", err);
    return apiError("Internal server error.", 500);
  }
}

/**
 * PUT /api/analytics/rules/events
 *
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (Object.keys(updateFields).length > 0) {
      ...
      if (error) {
        captureApiError("PUT /api/analytics/rules/events (set)", error);
    ...
    if (event_type_ids !== undefined) {
      ...
      if (junctionDelError) {
        captureApiError("PUT /api/analytics/rules/events (delete junctions)", junctionDelError);
      ...
      if (event_type_ids.length > 0) {
        ...
        if (junctionError) {
          captureApiError("PUT /api/analytics/rules/events (junctions)", junctionError);
    ...
    if (tiers) {
      ...
      if (delError) {
        captureApiError("PUT /api/analytics/rules/events (delete tiers)", delError);
      ...
      if (tiers.length > 0) {
        ...
        if (insError) {
          captureApiError("PUT /api/analytics/rules/events (insert tiers)", insError);
    ...
    captureApiError("PUT /api/analytics/rules/events", err);
    return apiError("Internal server error.", 500);
  }
}

/**
 * DELETE /api/analytics/rules/events
 *
...
export async function DELETE(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (error) {
      captureApiError("DELETE /api/analytics/rules/events", error);
      return apiError("Failed to delete rule set.", 500);
    }

    return NextResponse.json({ data: { deleted: true } });
  } catch (err) {
    captureApiError("DELETE /api/analytics/rules/events", err);
    return apiError("Internal server error.", 500);
  }
}
