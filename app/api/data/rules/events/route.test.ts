...
describe("GET /api/data/rules/events", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const req = createTestRequest("/api/data/rules/events", { searchParams: { clan_id: CLAN_ID } });
    const res = await GET(req);
    expect(res.status).toBe(401);
  });

  it("returns 400 for missing clan_id", async () => {
    const req = createTestRequest("/api/data/rules/events");
    const res = await GET(req);
...
describe("POST /api/data/rules/events", () => {
  let mockAuth: ReturnType<typeof createMockAuth>;
...
describe("PUT /api/data/rules/events", () => {
  let mockAuth: ReturnType<typeof createMockAuth>;
  ...
  it("returns 400 for invalid body", async () => {
    const req = createTestRequest("/api/data/rules/events", { method: "PUT", body: {} });
    const res = await PUT(req);
    expect(res.status).toBe(400);
  });
...
describe("DELETE /api/data/rules/events", () => {
  let mockAuth: ReturnType<typeof createMockAuth>;
