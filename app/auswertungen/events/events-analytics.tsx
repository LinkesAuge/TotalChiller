...
export default function EventsAnalytics(): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      try {
        const url = `/api/data/events?clan_id=${encodeURIComponent(clanId!)}&page=1&page_size=500`;
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      try {
        const url = `/api/data/events?clan_id=${encodeURIComponent(clanId!)}&event_id=${encodeURIComponent(selectedEventId!)}&page=1&page_size=500`;
        const res = await fetch(url);
        if (!res.ok) throw new Error(`${res.status}`);
        const json = await res.json();
  ...
  const handleSelectEvent = useCallback(
    (eventId: string) => {
      router.push(`/auswertungen/events?event=${encodeURIComponent(eventId)}`);
    },
    [router],
  );

  const handleBackToList = useCallback(() => {
    router.push("/auswertungen/events");
  }, [router]);
          ...
          {listData && (
            <>
              {(() => {
                ...
                const hasRanking = (listData.latest_event_ranking?.length ?? 0) > 0;
                return (
                  <div className={`analytics-events-top${hasRanking ? "" : " analytics-events-top--no-ranking"}`}>
                    {hasRanking && (
                      <div className="analytics-featured-event">
                        <div className="analytics-featured-event__header">
                          <h4>
                            <svg
                              viewBox="0 0 24 24"
                              fill="none"
                          ...
                          <span className="analytics-featured-event__date">
                    ...
                    <div className="analytics-events-sidebar">
                      {latestBarData.length > 0 && (
                        <div className="analytics-chart-wrapper">
                          <h4>
                            <svg
                              viewBox="0 0 24 24"
                      ...
                      <div className="analytics-summary-grid">
                        ...
                        <div className="analytics-summary-card">
                          ...
                          <span className="analytics-summary-card__value">
                            {listTotalParticipants.toLocaleString()}
                          </span>
                        ...
                        <section className="table best-performers">
                          ...
                          {listData.best_performers.map((p, idx) => (
                            <div className="row" key={p.game_account_id ?? `bp-${idx}`}>
                              <span>
                                <Link
                                  href={`/auswertungen/player?name=${encodeURIComponent(p.player_name)}${p.game_account_id ? `&ga=${encodeURIComponent(p.game_account_id)}` : ""}`}
                                  className="player-link"
                                >
                                  {p.player_name}
                                </Link>
