  ...
  return (
    <div style={p.page}>
      ...
      <aside className="v2a-sb" style={{ ...p.sb, width: w }}>
        ...
        <div style={{ ...p.logoArea, flexDirection: open ? "row" as const : "column" as const, padding: open ? "14px 14px 8px" : "14px 6px 8px" }}>
          <img src="/assets/vip/components_decor_12.png" alt="Clan" style={{ width: open ? 48 : 38, height: "auto", objectFit: "contain" as const, flexShrink: 0, filter: "drop-shadow(0 2px 6px rgba(201,163,74,0.4))" }} />
          {open && (
              ...
              <div key={group}>
                ...
                {gi > 0 && (
                  <div style={{ padding: open ? "4px 8px" : "4px 2px", display: "flex", justifyContent: "center" }}>
                    <img src="/assets/vip/components_decor_5.png" alt="" style={{ width: open ? "80%" : 40, height: "auto", opacity: 0.5 }} />
                  </div>
                )}
                ...
                {items.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="v2a-frame" style={p.iconFrame}>
                      ...
                      {item.vipIcon ? (
                        <img src={item.vipIcon} alt="" style={{ width: 16, height: 16, objectFit: "contain" as const, position: "relative" as const, zIndex: 1 }} />
                      ) : (
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={item.active ? "#e4c778" : "#9ab8d0"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ position: "relative" as const, zIndex: 1 }}><path d={item.icon} /></svg>
                      )}
                    </div>
                    {open && <span style={{ ...p.navLabel, color: item.active ? "#e4c778" : "#9a8b6f" }}>{item.label}</span>}
                    {item.badge !== undefined && (
                      <span style={{ ...p.badge, ...(open ? { marginLeft: "auto" } : { position: "absolute" as const, top: 0, right: 2, minWidth: 14, height: 14, fontSize: "0.5rem" }) }}>{item.badge}</span>
                    )}
                  </a>
                ))}
              </div>
        ...
        {/* Bottom: toggle + user */}
        <div style={p.sbBot}>
          <img src="/assets/vip/components_decor_6.png" alt="" style={{ width: "80%", height: "auto", opacity: 0.35, margin: "0 auto 6px", display: "block" }} />
          <button onClick={() => setOpen(!open)} style={{ ...p.togBtn, justifyContent: open ? "flex-start" : "center" }} className="v2a-toggle" aria-label="Toggle">
            <img src={open ? "/assets/vip/button_arrow_up_5.png" : "/assets/vip/button_arrow_up_6.png"} alt="" style={{ width: 16, height: 16, objectFit: "contain" as const, transform: open ? "rotate(-90deg)" : "rotate(90deg)" }} />
      ...
      <div className="v2a-main" style={{ ...p.main, marginLeft: w }}>
        ...
        <div style={p.hero}>
          ...
          <div style={{ position: "relative" as const, zIndex: 2, textAlign: "center" as const, display: "flex", flexDirection: "column" as const, alignItems: "center", gap: 4 }}>
            ...
            <h2 style={{ fontFamily: "'Fontin Sans', serif", fontSize: "1.6rem", fontWeight: 700, color: "#e4c778", textTransform: "uppercase" as const, margin: 0, letterSpacing: "0.06em", textShadow: "0 2px 16px rgba(201,163,74,0.4)" }}>Community Hub</h2>
            ...
            <img src="/assets/vip/components_decor_5.png" alt="" style={{ width: 180, opacity: 0.6, transform: "scaleY(-1)" }} />
          </div>
        </div>
        ...
        <div style={p.grid}>
          ...
          <section style={{ ...p.card, gridColumn: "span 2" }}>
            ...
            <div style={{ padding: "12px 18px" }}>
              {[
                ...
                <div key={i}>
                  {i > 0 && <div style={{ height: 1, background: "linear-gradient(90deg, transparent, rgba(201,163,74,0.3), transparent)" }} />}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 1, background: "rgba(201,163,74,0.15)" }}>
              {[
                ...
                <div key={i} style={{ background: "rgba(10,21,32,0.8)", padding: "14px 10px", textAlign: "center" as const }}>
                  <img src={stat.ico} alt="" style={{ width: 18, height: 18, margin: "0 auto 4px", display: "block", objectFit: "contain" as const }} />
          ...
          <section style={p.card}>
            ...
            {["Recruitment opens this week", "Alliance update posted", "War highlights published"].map((t, i) => (
              <div key={i} style={{ padding: "9px 16px", borderBottom: "1px solid rgba(45,80,115,0.2)", fontSize: "0.88rem", display: "flex", alignItems: "center", gap: 8 }}>
                <img src="/assets/vip/batler_icons_star_glow.png" alt="" style={{ width: 12, height: 12, opacity: 0.6 }} />
                {t}
              </div>
            ))}
          </section>
        ...
        <footer style={{ padding: "20px 24px", textAlign: "center" as const }}>
          <img src="/assets/vip/components_decor_5.png" alt="" style={{ width: 120, opacity: 0.3, margin: "0 auto 8px", display: "block" }} />
          <span style={{ color: "#5c5040", fontSize: "0.78rem" }}>The Chillers &bull; Community Hub</span>
        </footer>
      </div>

      <a href="/redesign" style={p.back}>&larr; Back to versions</a>
    </div>
  );
}
...
const p: Record<string, React.CSSProperties> = {
  ...
  logoArea: { display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid rgba(201,163,74,0.2)", flexShrink: 0 },
  logoT: { fontFamily: "'Fontin Sans', serif", fontSize: "0.95rem", fontWeight: 700, color: "#e4c778", whiteSpace: "nowrap" as const, textShadow: "0 0 12px rgba(228,199,120,0.2)" },
  nav: { flex: 1, overflowY: "auto" as const, padding: "4px 6px" },
  secLabel: { fontSize: "0.6rem", fontWeight: 700, color: "#5c5040", textTransform: "uppercase" as const, letterSpacing: "0.1em", padding: "6px 10px 2px", whiteSpace: "nowrap" as const },
  navItem: { display: "flex", alignItems: "center", gap: 10, textDecoration: "none", color: "#9a8b6f", borderRadius: 6, fontSize: "0.82rem", fontWeight: 500, whiteSpace: "nowrap" as const, overflow: "hidden" },
  navActive: { background: "rgba(201,163,74,0.1)", color: "#e4c778" },
  iconFrame: { width: 30, height: 30, borderRadius: "50%", position: "relative" as const, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.15s" },
  frameBg: { position: "absolute" as const, inset: 0, width: "100%", height: "100%", objectFit: "cover" as const, borderRadius: "50%" },
  ...
  togBtn: { display: "flex", alignItems: "center", gap: 6, width: "100%", padding: "5px 8px", borderRadius: 6, border: "none", background: "transparent", color: "#5c5040", cursor: "pointer", transition: "all 0.15s", marginBottom: 4 },
  userRow: { display: "flex", alignItems: "center", gap: 8, padding: "6px 6px", borderRadius: 6, overflow: "hidden" },
  avatarWrap: { width: 30, height: 30, borderRadius: "50%", position: "relative" as const, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  topBarBg: { position: "absolute" as const, inset: 0, width: "100%", height: "100%", objectFit: "cover" as const, opacity: 0.8 },
  topBarContent: { position: "relative" as const, zIndex: 1, padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" },
  topTitle: { fontFamily: "'Fontin Sans', serif", fontSize: "1.2rem", fontWeight: 700, color: "#e4c778", margin: 0, textShadow: "0 1px 6px rgba(0,0,0,0.5)" },
  ...
  goldBtn: { padding: "7px 14px", borderRadius: 8, border: "1px solid rgba(201,163,74,0.5)", background: "linear-gradient(180deg, rgba(201,163,74,0.15), rgba(201,163,74,0.05))", color: "#e4c778", fontSize: "0.8rem", fontWeight: 600, cursor: "pointer" },
  ...
  card: { background: "linear-gradient(180deg, rgba(22,44,66,0.9), rgba(14,30,45,0.92))", border: "1px solid rgba(45,80,115,0.5)", borderRadius: 12, overflow: "hidden" },
  cardHead: { position: "relative" as const, overflow: "hidden" },
  cardHeadBg: { position: "absolute" as const, inset: 0, width: "100%", height: "100%", objectFit: "cover" as const, opacity: 0.6 },
  cardHeadContent: { position: "relative" as const, zIndex: 1, display: "flex", alignItems: "center", gap: 8, padding: "10px 16px" },
