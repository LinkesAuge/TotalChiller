    ...
    <div style={s.page}>
      ...
      <aside className="v3b-sb" style={{ ...s.sb, width: w }}>
        ...
        <div style={{ ...s.sbHead, padding: open ? "14px 12px 10px" : "14px 6px 10px", justifyContent: open ? "flex-start" : "center" }}>
          ...
          {open && <div><div style={s.sbTitle}>The Chillers</div><div style={{ fontSize: "0.62rem", color: "#7a6e5a" }}>Alpha Division</div></div>}
        </div>
        ...
        <nav style={s.nav}>
          {GROUPS.map((group, gi) => {
            ...
            return (
              <div key={group}>
                {gi > 0 && <div style={{ margin: "4px 6px", height: 1, background: "linear-gradient(90deg, transparent, rgba(201,163,74,0.2), transparent)" }} />}
                {open && <div style={s.groupLabel}>{GROUP_LABELS[group]}</div>}
                {items.map((item, i) => {
                  const isActive = Boolean(item.active);
                  return (
                    <a key={i} href="#" className="v3b-link" data-tip={!open ? item.label : undefined} style={{ ...s.navItem, padding: open ? "7px 10px" : "7px 0", justifyContent: open ? "flex-start" : "center" }}>
                      ...
                      {item.badge !== undefined && <span style={{ ...s.badge, position: open ? "relative" as const : "absolute" as const, ...(open ? { marginLeft: "auto" } : { top: 1, right: 3, minWidth: 14, height: 14, fontSize: "0.5rem" }), zIndex: 1 }}>{item.badge}</span>}
                    </a>
                  );
                })}
              </div>
        ...
        <div style={s.sbBot}>
          ...
          <button onClick={() => setOpen(!open)} style={{ ...s.togBtn, justifyContent: open ? "flex-start" : "center" }} className="v3b-toggle" aria-label="Toggle">
          ...
          <div style={{ ...s.userRow, justifyContent: open ? "flex-start" : "center" }}>
            ...
            {open && <div style={{ overflow: "hidden" }}><div style={{ fontSize: "0.78rem", fontWeight: 600, color: "#f2e6c9" }}>DragonKnight99</div><div style={{ fontSize: "0.62rem", color: "#7a6e5a" }}>Officer</div></div>}
          </div>
        </div>
      </aside>
      ...
      <div className="v3b-main" style={{ ...s.main, marginLeft: w }}>
        ...
        <div style={s.grid}>
                ...
                <div key={i}>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "10px 0" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: "0.68rem", color: "#5c5040", marginTop: 3 }}>by {item.author} &bull; {item.time}</div>
                    </div>
                    <span style={{ padding: "2px 8px", borderRadius: 14, background: `linear-gradient(180deg, ${item.color}, ${item.color}cc)`, color: "#fff", fontSize: "0.6rem", fontWeight: 700, textTransform: "uppercase" as const, flexShrink: 0, marginTop: 2 }}>{item.tag}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section style={{ ...s.card, gridColumn: "span 2" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 1, background: "rgba(201,163,74,0.12)" }}>
              {[
                ...
                { v: "86%", l: "Event Readiness", ico: "/assets/vip/batler_icons_stat_heal.png", trend: "-2%", up: false },
              ].map((stat, i) => (
                <div key={i} style={{ background: "rgba(10,21,32,0.8)", padding: "14px 10px", textAlign: "center" as const }}>
                  ...
                  <div style={{ fontSize: "0.6rem", color: stat.up ? "#4a9960" : "#c94a3a", fontWeight: 600, marginTop: 2 }}>
                    {stat.up ? "▲" : "▼"} {stat.trend}
                  </div>
          ...
          <section style={s.card}>
            <div style={{ padding: "11px 16px", borderBottom: "1px solid rgba(45,80,115,0.4)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
